...
void print_good_set (double parameters[], double score, double cond_score, double exp_score) {
    if (ip.print_good_sets && score <= ip.good_set_threshold) {
        ...
        ip.good_sets_stream << score << "," << cond_score << "," << exp_score << ","<< parameters[0];
...
/* read_pipe reads the maximum score and the received score from the given pipe
	...
	todo:
*/
void read_pipe (int fd, double* score, double * cond_score, double * exp_score) {
	read_pipe_double(fd, score);
	read_pipe_double(fd, cond_score);
	read_pipe_double(fd, exp_score);
}
