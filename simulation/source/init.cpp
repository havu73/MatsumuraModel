	...
	if (num_args >1){
		for (int i = 1; i < num_args; i += 2){
			...
			if (option_set(option, "-i", "--params-file")) {
				...
				ip.compare_exp_data = true;
			}else if (option_set(option, "-th", "--diff-threshold")){
				ensure_nonempty(option, value);
				ip.exp_diff_thres = atoi(value);
				if (ip.exp_diff_thres < 0){
					usage("The threshold for average difference between experimental data and simulation data should be a positive number. Set -th or --diff-threshold to at least 0.");
				}
			}else if (option_set(option, "-pp", "--print-passed")){
				ensure_nonempty(option, value);
...
void get_mutant_data(input_params& ip, char* input){
	...
	while (result != NULL){
		...
		else if ((strcmp(result, "p1") == 0) || (strcmp(result, "1") == 0)){
			ip.mutants[index] = P1;
		}
		else if ((strcmp(result, "p2") == 0) || (strcmp(result, "2") == 0)){
			ip.mutants[index] = P2;
		}
		else if ((strcmp(result, "p3") == 0) || (strcmp(result, "3") == 0)){
			ip.mutants[index] = P3;
		}
		else if ((strcmp(result, "c1") == 0) || (strcmp(result, "4") == 0)){
			ip.mutants[index] = C1;
		}
		else if ((strcmp(result, "c2") == 0) || (strcmp(result, "5") == 0)){
			ip.mutants[index] = C2;
		}
		else if ((strcmp(result, "bm") == 0) || (strcmp(result, "6") == 0)){
			ip.mutants[index] = BM;
		}
		else if ((strcmp(result, "np") == 0) || (strcmp(result, "7") == 0)){
			ip.mutants[index] = NP;
		}
		else if ((strcmp(result, "c1c2") == 0) || (strcmp(result, "8") == 0)){
			ip.mutants[index] = C1C2;
		}
		else if ((strcmp(result, "p2c1") == 0) || (strcmp(result, "9") == 0)){
			ip.mutants[index] = P2C1;
		}
		else if ((strcmp(result, "p2c2") == 0) || (strcmp(result, "10") == 0)){
			ip.mutants[index] = P2C2;
		}
		else if ((strcmp(result, "rev") == 0) || (strcmp(result, "11") == 0)){
			ip.mutants[index] = R;
		}
		else {
		...
		if (ip.cell_type == SCN && ip.mutants[index] >= NUM_SCN_MUTANTS){
			usage("The program only supports 9 mutants for SCN cell");
		}
	...
	if (ip.cell_type == SCN){
		for (int i = 0; i < ip.num_mutants; i ++){
			if (ip.mutants[i] == WT){
				...
				usage("Mutant indices are erroneous");
			}
		}
	}
...
void get_cell_type(input_params& ip, char* input){
	if (strcmp(input, "scn") == 0){
		...
		cout << term->red << "Invalid cell type: " << term->reset << input << endl;
		usage("We can only accept cell types: 'scn', 'fib'");
	}
}
...
void read_experiment_data(input_params& ip, input_data& experiment_data, exp_data& ed){
	...
	for (int i = 0; i < ed.num_time_steps; i ++){
		parse_experiment_data_line(ip, experiment_data, ed, i);
	}
}
...
void update_initial_conditions(con_levels& cl){
	...
	//////ADDED INITIAL CONDTIONS TO KIM AND FORGER'S MODEL
	cl.data[MNPH][0] = 6.26955;
	cl.data[MCPH][0] = 0.995394;
	cl.data[GPH][0] = 0.210048;	//same as G
	cl.data[GPHR][0] = 0.787955; //same as GRB
	cl.data[X70000][0] = 0.6686315;
	cl.data[X80000][0] = 0.54576475;
	cl.data[X80100][0] = 0.664039;
	cl.data[X90000][0] = 0.0033524383333333337;
	cl.data[X90010][0] = 7.868643333333333e-05;
	cl.data[X90011][0] = 0.020329527666666666;
	cl.data[X90100][0] = 0.10487046666666666;
	cl.data[X90110][0] = 0.0005949053333333333;
	cl.data[X90111][0] = 0.04448596666666666;
	...
	cl.data[GPO][0] = 0.210048;	//G
	cl.data[GPTR][0] = 0.787955; //GRB
